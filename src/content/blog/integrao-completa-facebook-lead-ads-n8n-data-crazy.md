...
Criar App → Selecione “Outro” → “Business”
Nome do App: Facebook Lead Ads - Habilitar Sorriso (Sorriso-MT)
Email de contato: Seu email
Casos de uso: Outro (para começar zerado)
Tipo: Business/Empresarial
Portfólio Empresarial: BM Empresa, Apps (selecione sua BM)

2. Configurar o App
Em “Login do Facebook para Empresas”:

Clique em Configurar
Em “URLs de redirecionamento válidos”, adicione:
...
https://www.facebook.com/privacy/policy/?locale=pt_BR
(Use a mesma URL da política de privacidade)
...
Você verá um botão “Verificar sua empresa”
Clique e preencha:
...
Você receberá email da Meta confirmando
...
Vá em Análise do App > Permissões e Recursos
Localize leads_retrieval (ou public_advanced)
Clique em Solicitar Acesso Avançado
Selecione: BM Empresa, Apps
Clique em Concordar
...
Você receberá email: “Renovação de acesso concluída”
No canto superior direito do app, clique no botão
Mude de Desenvolvimento para Ao Vivo
Se pedir nova avaliação, preencha o mesmo formulário (não, não, não, não)
Aguarde mais ~1 hora se necessário
...
No n8n (work.novoenvio.com.br), vá em Credenciais
Clique em Adicionar Credencial
Busque: Facebook Lead Ads OAuth2 API
Preencha:
...
Importante: A URL de callback já aparece automaticamente:

https://work.novoenvio.com.br/rest/oauth2-credential/callback
(Essa mesma URL você já colocou no Facebook)


Clique em Conectar minha conta


Faça login no Facebook


CRÍTICO: Na tela de permissões, selecione:
...
Aguarde a tela verde de sucesso no n8n
...
Acesse a API do Data Crazy: GET /v1/pipelines
Ou veja na URL do CRM quando estiver no funil
...
ColunaValorData/Hora{{$json.processed_at}}Ação{{$json.acao_portugues}}Nome{{$json.lead_name}}Telefone{{$json.lead_phone}}Email{{$json.lead_email}}Cidade{{$json.lead_cidade}}Estado{{$json.lead_estado}}Campanha{{$json.campaign}}Conjunto Anúncios{{$json.adset}}Anúncio{{$json.ad}}Lead ID{{$json.lead_id}}Deal ID{{$json.deal_id}}Tempo (seg){{$json.tempo_processamento_seg}}Facebook Lead ID{{$json.facebook_lead_id}}Execution ID{{$json.workflow_execution_id}}
...
Acesse: https://developers.facebook.com/tools/lead-ads-testing/
...
⚠️ NÃO clique em “Criar Lead” (dados aleatórios)


Clique em “Ver Prévia do Formulário”
...
Vá para o n8n → Execuções → Veja a execução aparecer
...
Acesse o CRM: https://datacrazy.com.br
Busque pelo telefone: +5565999999999
Verifique:
...
Na ferramenta do Facebook, clique em “Excluir Lead”
Preencha o formulário novamente com o mesmo telefone
Mas mude o nome: Diego Carmona Teste 2
Envie
Verifique no n8n:
...
Criar app → Imediato
Verificação BM → 1-3 dias
Acesso avançado → 1 hora a 24h
Renovação → A cada 90 dias (email avisa)
...
Taxa de Conversão Formulário → CRM
...
ABCDEFGHIJKData/HoraAçãoNomeTelefoneEmailCidadeEstadoCampanhaQuality ScoreTempo (s)Lead ID
Fórmulas úteis:
// Total de leads hoje
=COUNTIF(A:A, ">="&#x26;TODAY())
...
Workflow não está ativado (produção)
Credencial desconectada
App do Facebook não está “Ao Vivo”
...
Crie o app no Facebook
Aguarde aprovação (1-24h)
